...
import static unimelb.bitbox.ServerMain.peerMap;
...
public class SocketHandler implements Runnable {
    ...
    public interface OperationMonitor {
        ...
        void run(Document request, DataOutputStream output) throws Exception;
    ...
    private OperationMonitor optHandshakeReq = (request, output) -> {
        ...
        // check pool size
        if (peerMap.size() + 1 > Integer.parseInt(Configuration.getConfigurationValue("maximumIncommingConnections"))) {
            // full, close
            re.append("command", Operation.CONNECTION_REFUSED.name());
            re.append("message", "connection limit reached");
            ArrayList<Document> documents = new ArrayList<>(peerMap.size());
            ...
            log.info(String.valueOf(ServerMain.peerMap) + " " + ip);
